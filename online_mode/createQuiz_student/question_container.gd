...
export(NodePath) onready var incorrect_content = get_node(incorrect_content) as Label
...
func _on_CheckBox_toggled(button_pressed):
	...
	if button_pressed:
		if !already_selected(question_content.text):
			...
			new_selected.find_node("incorrect_content").text = incorrect_content.text
...
func already_selected(question):
	for child in selected_container.get_children():
		if child.find_node("question_content").text == question:
			return true
	return false
